...
import fitz  # PyMuPDF
import io
# Flowクラスを使うことで、エラーの原因であるredirect_uriを確実に制御します
...
# --- 認証処理の関数（修正：redirect_uriを明示的に指定） ---
def authenticate_google():
    ...
    if not creds or not creds.valid:
        ...
        if not creds:
            ...
            # Flowクラスを使い、redirect_uriを明示的に指定して初期化します
            ...
            auth_response = st.text_input("**手順3:** コピーしたURL（http://localhost/?code=...）をここに貼り付けてEnter:")
            
            if auth_response:
                try:
                    # 入力されたURLから認証コードを取り出して、アクセス許可（トークン）を取得します
                    flow.fetch_token(authorization_response=auth_response)
                    creds = flow.credentials
                    st.session_state.google_creds = creds
                    st.success("認証に成功しました！ページを自動的に更新します...")
                    st.rerun()
